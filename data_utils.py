...
class Dataset_parallel:
    def __init__(
        ...
        # Delete the bsatches dictionary since it won't be needed anymore
        del batches_dict
        gc.collect()
...
def get_results_df(
    ...
    print(f"execution times are : ,{df['exec_time']}")
    ma = 0
    max_id =0
    for i in range(0,len(df['exec_time'])):
         if df['exec_time'][i]>ma:
             max_id=i
             ma=df['exec_time'][i]
    print(f"Maximum speed up index and execution time are: {max_id, ma}")
    return df
